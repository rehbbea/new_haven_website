<?php
namespace Eldritch\Modules\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function __construct() {
		$this->base = 'edgt_pie_chart_doughnut';
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__('Chart Value', 'edge-core') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type'       => 'colorpicker',
				'heading'    => esc_html__('Chart Color', 'edge-core') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__('Chart Legend', 'edge-core') . $i,
		...
		vc_map(array(
			'name'                      => esc_html__('Pie Chart 3 (Doughnut)', 'edge-core'),
			'base'                      => $this->getBase(),
			...
			'params'                    => array_merge(
				array(
					...
					array(
						'type'       => 'textfield',
						'heading'    => esc_html__('Height', 'edge-core'),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = edge_core_get_core_shortcode_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
