...
class EldritchEdgeWoocommerceDropdownCart extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		?>
		<div class="edgt-shopping-cart-holder <?php echo esc_attr( $icon_class ); ?>" <?php eldritch_edge_inline_style( $icon_styles ) ?>>
			<?php if ( is_object( WC()->cart ) ) { ?>
				<div class="edgt-shopping-cart-inner">
					...
					<span class="edgt-cart-icon icon_bag_alt"><span
						...
						<span
								class="edgt-cart-info-total"><?php echo wp_kses( WC()->cart->get_cart_subtotal(), array(
								'span' => array(
									'class' => true,
									'id'    => true
								)
									...
									<li>
										...
										<div class="edgt-item-info-holder">
											<h5 itemprop="name" class="edgt-product-title"><a itemprop="url"
											                                                  href="<?php echo esc_url( get_permalink( $cart_item['product_id'] ) ); ?>"><?php echo apply_filters( 'woocommerce_cart_item_name', $_product->get_title(), $_product ); ?></a>
											</h5>
											<span
													class="edgt-quantity"><?php echo esc_html( $cart_item['quantity'] ); ?></span>
											<?php echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); ?>
											<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s"><span arie-hidden="true" class="icon_close"></span></a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_attr__( 'Remove this item', 'eldritch' ) ), $cart_item_key ); ?>
										</div>
									</li>
								<?php endforeach; ?>
						...
						<?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
							...
							<li>
								...
								<div class="edgt-item-info-holder">
									...
									<?php echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); ?>
									<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s"><span aria-hidden="true" class="icon_close"></span></a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_attr__( 'Remove this item', 'eldritch' ) ), $cart_item_key ); ?>
								</div>
							</li>
						<?php endforeach; ?>
						<li class="edgt-cart-bottom">
